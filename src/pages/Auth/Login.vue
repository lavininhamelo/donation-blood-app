<template>
  <div class="form-login full-width text-center">
    <header class>
      <span id="title-name" class="block full-width">iBlood</span>
      <small>Doe vida, doe sangue!</small>
    </header>
    <div class="q-px-xl full-width">
      <form id="form" v-on:submit.prevent="login()">
        <q-input outlined v-model="email" label="Email" class="q-mb-md" bg-color="white">
          <template v-slot:prepend>
            <q-icon color="primary" name="o_email" size="15px" />
          </template>
        </q-input>
        ...
        >
          <template v-slot:prepend>
            <q-icon color="primary" name="o_lock" size="15px" />
          </template>
        </q-input>
        <div class="row q-mb-md q-mt-none" style="float:right">
